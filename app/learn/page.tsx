...
export default function LearnPage(){
  return <AppShell>
    <div className="pageContainer">
      ...
      <div className="lessonGrid">{lessons.map(({icon:Icon,title,text,tag},i)=><article className="lessonCard" key={title}><div className="lessonIcon"><Icon size={21}/></div><small>{tag}</small><h2>{title}</h2><p>{text}</p></article>)}</div>
