...
begin
  ...
  v_username := lower(coalesce(nullif(trim(new.raw_user_meta_data ->> 'username'),''), split_part(coalesce(new.email,'student'),'@',1)));
  ...
  values(new.id,v_username,left(v_name,80),v_role)
...
select
  u.id,
  left(regexp_replace(lower(coalesce(nullif(trim(u.raw_user_meta_data ->> 'username'),''),split_part(coalesce(u.email,'student'),'@',1))),'[^a-z0-9_]+','','g'),24),
